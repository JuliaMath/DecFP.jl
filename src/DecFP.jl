...
for w in (32,64,128)
    BID = @compat(Symbol)(string("Dec",w))
...
for w in (32,64,128)
    BID = @compat(Symbol)(string("Dec",w))
    T = eval(BID)
    Ti = eval(@compat(Symbol)(string("UInt",w)))
    ...
    for w′ in (32,64,128)
        ...
        if w′ <= 64
            FP′ = @compat(Symbol)(string("Float",w′))
            @eval promote_rule(::Type{$BID}, ::Type{$FP′}) = $(@compat(Symbol)(string("Dec",max(w,w′))))
            for i′ in ("Int$w′", "UInt$w′")
                Ti′ = eval(@compat(Symbol)(i′))
                @eval begin
